...
import { FaPlaneDeparture, FaPlaneArrival, FaPlane, FaPassport, FaUser, FaDollarSign, FaPhone, FaTimes, FaClock, FaCalendarAlt, FaMapMarkerAlt } from 'react-icons/fa';
import { MdEventSeat, MdAttachMoney, MdMarkEmailUnread, MdAirplaneTicket, MdFlightTakeoff, MdFlightLand } from 'react-icons/md';

interface ResultPageProps {
  flightDetails: FlightDetails;
}

function FlightTimeline({ departure, stopover, arrival }: { departure: any; stopover?: any; arrival: any }) {
  return (
      ...
      <div className="hidden md:flex items-center justify-between relative px-4">
        ...
        <div className="relative z-10 flex flex-col items-center text-center bg-white px-3">
          ...
          <div className="mt-2 px-3 py-1.5 bg-gray-50 rounded-lg">
            ...
            {departure.airport && <p className="text-[10px] text-gray-400 mt-0.5">{departure.airport}</p>}
          </div>
        </div>
        ...
        {stopover && (
          <div className="relative z-10 flex flex-col items-center text-center bg-white px-3">
            ...
            <div className="mt-2 px-3 py-1.5 bg-gray-50 rounded-lg">
              ...
              {stopover.airport && <p className="text-[10px] text-gray-400 mt-0.5">{stopover.airport}</p>}
            </div>
          </div>
        )}
        ...
        <div className="relative z-10 flex flex-col items-center text-center bg-white px-3">
          ...
          <div className="mt-2 px-3 py-1.5 bg-gray-50 rounded-lg">
            ...
            {arrival.airport && <p className="text-[10px] text-gray-400 mt-0.5">{arrival.airport}</p>}
          </div>
        </div>
      </div>
      ...
      <div className="md:hidden space-y-0">
        ...
        <div className="flex items-start gap-4 relative">
          ...
          <div className="pb-6 flex-1">
            ...
            <p className="font-bold text-gray-800">
              {departure.time} &middot; {departure.date}
            </p>
            <p className="text-sm text-gray-600 mt-1">
              {departure.airportCode} &mdash; {departure.city}
            </p>
            {departure.airport && <p className="text-xs text-gray-400 mt-0.5">{departure.airport}</p>}
          </div>
        </div>
        ...
        {stopover && (
          <div className="flex items-start gap-4 relative">
            ...
            <div className="pb-6 flex-1">
              ...
              {stopover.airport && <p className="text-xs text-gray-400 mt-0.5">{stopover.airport}</p>}
            </div>
          </div>
        )}
        ...
        <div className="flex items-start gap-4 relative">
          ...
          <div className="flex-1">
            <p className="text-xs text-gray-500 uppercase tracking-wider font-medium">Arrival</p>
            <p className="font-bold text-gray-800">
              {arrival.time} &middot; {arrival.date}
            </p>
            <p className="text-sm text-gray-600 mt-1">
              {arrival.airportCode} &mdash; {arrival.city}
            </p>
            {arrival.airport && <p className="text-xs text-gray-400 mt-0.5">{arrival.airport}</p>}
          </div>
        </div>
      </div>
    </div>
...
function InfoRow({ icon: Icon, iconColor, label, value, bold }: { icon: React.ElementType; iconColor: string; label: string; value: string; bold?: boolean }) {
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-gray-100 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className={`text-sm ${bold ? 'font-bold text-gray-900 text-base' : 'font-semibold text-gray-800'}`}>{value}</p>
      </div>
    </div>
  );
}

function ContactRow({ icon: Icon, iconColor, label, value }: { icon: React.ElementType; iconColor: string; label: string; value: string }) {
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-gray-100 last:border-0">
      <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${iconColor}`}>
        <Icon className="w-4 h-4" />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-100 to-slate-200 py-6 px-4 sm:px-6">
      <div className="max-w-[860px] mx-auto">
            ...
            <div className="px-6 py-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-white rounded-xl p-2 flex items-center justify-center flex-shrink-0">
                  <Image src={airlineLogo} width={200} height={200} alt={airlineName || 'Airline Logo'} className="w-full h-full object-contain" />
                </div>
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    ...
                    {returnFlight && (
                      <>
                        {' '}
                        &middot; {returnFlight.departure.city} ({returnFlight.departure.airportCode}) &rarr; {returnFlight.arrival.city} ({returnFlight.arrival.airportCode})
                      </>
                    )}
                  </p>
                </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <Link href="/" className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 transition-colors text-white text-sm font-semibold py-2 px-4 rounded-lg">
          ...
          <div className="px-6 py-6 space-y-7">
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-slate-50 rounded-xl p-5 border border-slate-100">
                  ...
                  <div className="space-y-0">
                    {passengerInfo.name && <InfoRow icon={FaUser} iconColor="bg-slate-100 text-slate-600" label="Full Name" value={passengerInfo.name} />}
                    {passengerInfo.dateOfBirth && <InfoRow icon={FaCalendarAlt} iconColor="bg-slate-100 text-slate-600" label="Date of Birth" value={passengerInfo.dateOfBirth} />}
                    {passengerInfo.motherName && <InfoRow icon={FaUser} iconColor="bg-slate-100 text-slate-600" label="Mother's Name" value={passengerInfo.motherName} />}
                    {passengerInfo.motherDob && <InfoRow icon={FaCalendarAlt} iconColor="bg-slate-100 text-slate-600" label="Mother's DOB" value={passengerInfo.motherDob} />}
                    {passengerInfo.sonName && <InfoRow icon={FaUser} iconColor="bg-slate-100 text-slate-600" label="Son's Name" value={passengerInfo.sonName} />}
                    {passengerInfo.sonDob && <InfoRow icon={FaCalendarAlt} iconColor="bg-slate-100 text-slate-600" label="Son's DOB" value={passengerInfo.sonDob} />}
                  </div>
                </div>
                ...
                <div className="bg-slate-50 rounded-xl p-5 border border-slate-100">
                  ...
                  <div className="space-y-0">
                    ...
                    />
                    <InfoRow icon={MdEventSeat} iconColor="bg-indigo-50 text-indigo-600" label="Seat Number" value={passengerInfo.seatNumber} />
                    <InfoRow icon={FaUser} iconColor="bg-indigo-50 text-indigo-600" label="Passenger Type" value={passengerInfo.passengerType} />
                    <InfoRow icon={FaUser} iconColor="bg-indigo-50 text-indigo-600" label="Class" value={passengerInfo.class} />
                  </div>
                </div>
                ...
                <div className="bg-slate-50 rounded-xl p-5 border border-slate-100 md:col-span-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-0">
                    <ContactRow icon={MdMarkEmailUnread} iconColor="bg-emerald-50 text-emerald-600" label="Email" value={passengerInfo.contactInfo.email} />
                    <ContactRow icon={FaPhone} iconColor="bg-emerald-50 text-emerald-600" label="Phone" value={passengerInfo.contactInfo.phone} />
                  </div>
                </div>
              </div>
            </section>
            ...
            {returnFlight && (
              <section>
                ...
                <p className="text-xs hidden text-gray-500 mb-4 ml-4">
                ...
                <div className="bg-slate-50 rounded-xl p-5 border border-slate-100">
                  <FlightTimeline departure={returnFlight.departure} stopover={returnFlight.stopover} arrival={returnFlight.arrival} />
                </div>
                ...
                {returnFlight.passengerInfo && (
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-slate-50 rounded-xl p-5 border border-slate-100">
                      ...
                      <div className="space-y-0">
                        {returnFlight.passengerInfo.name && <InfoRow icon={FaUser} iconColor="bg-slate-100 text-slate-600" label="Full Name" value={returnFlight.passengerInfo.name} />}
                        <InfoRow icon={MdEventSeat} iconColor="bg-slate-100 text-slate-600" label="Seat Number" value={returnFlight.passengerInfo.seatNumber || '—'} />
                        <InfoRow icon={FaUser} iconColor="bg-slate-100 text-slate-600" label="Passenger Type" value={returnFlight.passengerInfo.passengerType || '—'} />
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-50 rounded-xl p-5 border border-slate-100 md:col-span-2">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-0">
                        <ContactRow icon={MdMarkEmailUnread} iconColor="bg-emerald-50 text-emerald-600" label="Email" value={returnFlight.passengerInfo.contactInfo.email} />
                        <ContactRow icon={FaPhone} iconColor="bg-emerald-50 text-emerald-600" label="Phone" value={returnFlight.passengerInfo.contactInfo.phone} />
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="mt-4 bg-gradient-to-r from-slate-50 to-slate-100 rounded-xl p-5 border border-slate-200">
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      ...
                      <p className="text-lg font-bold text-gray-800 mt-1">${pricing.ticketPrice.toFixed(2)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wider">Taxes & Fees</p>
                      <p className="text-lg font-bold text-gray-800 mt-1">${pricing.taxes.toFixed(2)}</p>
                    </div>
                    <div className="border-l border-slate-200 pl-4">
                      <p className="text-xs text-gray-500 uppercase tracking-wider">Total</p>
                      <p className="text-2xl font-extrabold text-[#11172b] mt-1">${pricing.total.toFixed(2)}</p>
                    </div>
                  </div>
                </div>
              </section>
          ...
          <div className="bg-gradient-to-r from-[#11172b] to-[#1a2744] px-6 py-4">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
              ...
              <div className="text-center sm:text-right">
                ...
                <p className="text-white font-mono font-bold text-sm tracking-wider">{passengerInfo.confirmationNumber || passengerInfo.ticketNumber || '—'}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Bottom disclaimer */}
        <p className="text-center text-xs text-gray-400 mt-4">This document is for informational purposes only. Please check with your airline for the most up-to-date flight information.</p>
      </div>
    </div>
  );
}
